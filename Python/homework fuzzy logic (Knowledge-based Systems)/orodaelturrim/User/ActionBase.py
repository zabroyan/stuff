...
class ActionBase(IActionBase):
    ...
    * self.player [PlayerTag] instance of your player for identification yourself in proxy
    ...
    def spawn_archer(self, distance, tile):
        # print(distance)
        base = self.map_proxy.get_bases_positions().pop()
        distance = float(distance)
        for pos in tile:
            if distance - 1 <= base.distance_from(pos) <= distance + 1:
                # print("archer at", pos)
                self.spawn(GameObjectType.ARCHER, pos)
                break

    def spawn_knight(self, distance, tile):
        # print(distance)
        base = self.map_proxy.get_bases_positions().pop()
        distance = float(distance)
        for pos in tile:
            if distance - 1 <= base.distance_from(pos) <= distance + 1:
                self.spawn(GameObjectType.KNIGHT, pos)
                break
